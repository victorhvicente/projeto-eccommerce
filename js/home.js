document.addEventListener('DOMContentLoaded', function() {
    ...
    const visibleProducts = Math.floor(productsTrack.clientWidth / productCardWidth);
    const maxPosition = productCards.length - visibleProducts;
    
    function updateProductsPosition() {
        productsTrack.style.transform = `translateX(${-productsPosition * productCardWidth}px)`;
    }
    ...
    const testimonialsPrevButton = document.querySelector('.testimonials-control.prev');
    const testimonialsNextButton = document.querySelector('.testimonials-control.next');
    let testimonialsPosition = 0;
    const testimonialWidth = testimonials[0].offsetWidth + 30; // Width + gap
    const visibleTestimonials = Math.floor(testimonialsTrack.clientWidth / testimonialWidth);
    const maxTestimonialsPosition = testimonials.length - visibleTestimonials;
    
    function updateTestimonialsPosition() {
        testimonialsTrack.style.transform = `translateX(${-testimonialsPosition * testimonialWidth}px)`;
    }
    ...
    function showNotification(message, type = 'success') {
        ...
        notification.style.backgroundColor = type === 'success' ? '#00e676' : '#e53935';
